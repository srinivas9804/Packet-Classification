...
void readFile(char* fileName) {
	...
	number_of_rules = i;
	if (originalTable[number_of_rules - 1].SA_BITMASK != 0 || originalTable[number_of_rules - 1].DA_BITMASK != 0 ||
		!(originalTable[number_of_rules - 1].SP_START == 0 && originalTable[number_of_rules - 1].SP_END == 65535) ||
		!(originalTable[number_of_rules - 1].DP_START == 0 && originalTable[number_of_rules - 1].DP_END == 65535) ||
		originalTable[number_of_rules - 1].PT_BITMASK != 0) {
		originalTable[number_of_rules].SA_BITMASK = 0;
		originalTable[number_of_rules].DA_BITMASK = 0;
		originalTable[number_of_rules].PT_BITMASK = 0;
		originalTable[number_of_rules].SP_START = 0;
		originalTable[number_of_rules].SP_END = 65535;
		originalTable[number_of_rules].DP_START = 0;
		originalTable[number_of_rules].DP_END = 65535;
		number_of_rules++;
	}
}
...
void assignPriority() {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < index; i++) {
			if (isCompatible(table[index].bv,table[index].bm,table[i].bv,table[i].bm)){
				int temp = value + 1;
				if (temp > table[i].priority) {
			...
			for (int j = 0; j < number_of_rules; j++) {
				...
				if (table[i].codeword_SA == table[j].codeword_SA && table[i].codeword_DA == table[j].codeword_DA
					...
					while (!temp_queue.empty()) {//BFS
						...
						temp_queue.pop();
						for (int k = 0; k < number_of_rules; k++) {
							if (isCompatible(table[index].bv, table[index].bm, table[k].bv, table[k].bm)) {
								if (k == i) {
									found = true;
									break;
								}
		...
		if (table[i].mask_PT == 0) {//wildcard protocol
			...
			for (int j = 0; j < number_of_rules; j++) {
				...
				if (table[i].codeword_SA == table[j].codeword_SA && table[i].codeword_DA == table[j].codeword_DA
					...
					&& table[i].mask_SA == table[j].mask_SA && table[i].mask_DA == table[j].mask_DA
					&& table[i].mask_SP == table[j].mask_SP && table[i].mask_DP == table[j].mask_DP) {
					if (isCompatible(table[i].bv, table[i].bm, table[j].bv, table[j].bm)) {
			...
			for (int j = 0; j < number_of_rules; j++) {
				...
				if (table[i].codeword_SA == table[j].codeword_SA && table[i].codeword_DA == table[j].codeword_DA
					...
					while (!temp_queue.empty() && flag) {
						...
						temp_queue.pop();
						for (int k = 0; k < number_of_rules ; k++) {
							if (isCompatible(table[k].bv, table[k].bm, table[index].bv, table[index].bm)) {
								if (k == i) {
									if (new_dist == 2) {
										case3_rules++;
	...
	for (list<pair<int, int> >::iterator it = case2_indices.begin(); it != case2_indices.end(); it++) {
		if (rules[it->first] == 0 && rules[it->second] == 0) {
			...
			while (!temp_queue.empty()) {
				...
				for (int i = 0; i < number_of_rules; i++) {
					if (isCompatible(table[index].bv, table[index].bm, table[i].bv, table[i].bm) && new_priority > table[i].priority) {
	...
	for (list<pair<int, int> >::iterator it = case1_indices.begin(); it != case1_indices.end(); it++) {
		int index = it->second;
		table[index].redundant_flag = true;
	}
...
int one_level_bit_selection(list<int> &indices) {//returns number of memory_units
	...
	int *address_arr = new int[number_of_rules];
	for (int j = 0; j < number_of_rules; j++) {
		address_arr[j] = selected_index(combinations[0], table[j].bv, table[j].bm);
	}
	do {
		int unit[1024][2];
		...
		if ((rule_ctr / 2048.0) >= threshold) {
			if (combination_ctr > 1 && (rule_ctr / 2048.0) < 0.4) {
				i++;
				combination_ctr = 0;
				for (list<int>::iterator it = temp_indices.begin(); it != temp_indices.end(); it++) {
					address_arr[*it] = selected_index(combinations[i], table[*it].bv, table[*it].bm);
				}
			}
			else {
...
int two_level_bit_selection(list<int> &indices) {
	...
	while (!indices.empty()) {
		...
		for (list<int>::iterator it = indices.begin(); it != indices.end(); it++) {
			for (int i = codelenPT; i < encodedRuleLen; i++) {
		...
		for (int i = codelenPT; i < encodedRuleLen; i++) {
			...
			if (BF[i] > largest || (BF[i]==largest && rs[i]>rs[b0])) {
				largest = BF[i];
				b0 = i;
			}
		}
		...
		largest = 0.0;
		for (int i = codelenPT; i < encodedRuleLen; i++) {
			if (BF[i] > largest || (BF[i] == largest && rs[i]>rs[b1])) {
				largest = BF[i];
				b1 = i;
			}
		}
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 8 && flag; j++) {
				...
				for (int k = 0; k < pow(2, j); k++) {
					...
					for (int l = codelenPT; l < encodedRuleLen; l++) {
						if(rules[i].size() > 0)
							BF[l] += (((double)min(rc0[l], rc1[l])) / rules[i].size());
					}
				}
				largest = 0;
				for (int l = codelenPT; l < encodedRuleLen; l++) {
...
int selected_index(int selected_bits[4], const unsigned long long int &bv, const unsigned long long int &bm) {// if -1 is returned, at least one of the bits is a wildcard 
	...
	for (int i = 0; i < 4; i++) {
		for (int j = selected_bits[i]-1; j >= 0;j--) {
			if ((bm >> (arr[i] + j) & 1) == 0) {
				return -1;
			}
			else {
